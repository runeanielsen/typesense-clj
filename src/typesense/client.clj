...
(defn create-collection!
  ...
  (let [{:keys [uri req]} (api/create-collection-req settings schema)]
    (handle-json-response (http/post uri req))))
...
(defn drop-collection!
  ...
  (let [{:keys [uri req]} (api/drop-collection-req settings collection-name)]
    (handle-json-response (http/delete uri req))))
...
(defn list-collections
  ...
  (let [{:keys [uri req]} (api/list-collections-req settings)]
    (handle-json-response (http/get uri req))))
...
(defn update-document!
  ...
  (let [{:keys [uri req]} (api/update-document-req settings collection-name id document)]
    (handle-json-response (http/patch uri req))))
...
(defn import-documents!
  ...
  ([settings collection-name documents parameters]
   (let [{:keys [uri req]} (api/import-documents-req settings collection-name documents parameters)]
     (handle-jsonline-response (http/post uri req)))))
...
(defn export-documents
  ...
  (let [{:keys [uri req]} (api/export-documents-req settings collection-name parameters)]
    (handle-jsonline-response (http/get uri req))))
...
(defn upsert-override!
  ...
  (let [{:keys [uri req]} (api/upsert-override-req settings collection-name override-name override)]
    (handle-json-response (http/put uri req))))
