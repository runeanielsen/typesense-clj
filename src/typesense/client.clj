...
(defn create-collection!
  ...
  [settings schema]
  (let [{uri :uri req :req} (api/create-collection-req settings schema)]
...
(defn drop-collection!
  ...
  (let [{uri :uri req :req} (api/drop-collection-req settings collection-name)]
...
(defn retrieve-collection!
  ...
  [settings collection-name]
  (let [{uri :uri req :req} (api/retrieve-collection-req settings collection-name)]
    (handle-json-response (client/get uri req))))

(defn create-document!
  "Indexes the document."
  [settings collection-name document]
  (let [{uri :uri req :req} (api/create-document-req settings collection-name document)]
    (handle-json-response (client/post uri req))))

(defn upsert-document!
  "Indexes the document."
  [settings collection-name document]
  (let [{uri :uri req :req} (api/upsert-document-req settings collection-name document)]
    (handle-json-response (client/post uri req))))

(defn retrieve-document!
  "Retrieves the document on id in the specified collection."
  [settings collection-name id]
  (let [{uri :uri req :req} (api/retrieve-document-req settings collection-name id)]
    (handle-json-response (client/get uri req))))

(defn delete-document!
  "Deletes the document on id in the specified collection."
  [settings collection-name id]
  (let [{uri :uri req :req} (api/delete-document-req settings collection-name id)]
    (handle-json-response (client/delete uri req))))
...
(defn update-document!
  ...
  (let [{uri :uri req :req} (api/update-document-req settings collection-name id document)]
...
(defn import-documents!
  ...
  ([settings collection-name documents parameters]
   (let [{uri :uri req :req} (api/import-documents-req settings collection-name documents parameters)]
     (handle-jsonline-response (client/post uri req)))))

(defn delete-documents!
  "Delete documents."
  [settings collection-name parameters]
  (let [{uri :uri req :req} (api/delete-documents-req settings collection-name parameters)]
    (handle-json-response (client/delete uri req))))

(defn export-documents!
  "Exports documents in the specified collection."
  [settings collection-name parameters]
  (let [{uri :uri req :req} (api/export-documents-req settings collection-name parameters)]
